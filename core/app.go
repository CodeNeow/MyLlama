...
func (a *App) GetConfig() map[string]interface{} {
	...
	return map[string]interface{}{
		...
		// Effective LoRA adapter directory (default <model download dir>/lora),
		// surfaced the same way for the model-settings empty-state hint.
		"loraDir":             absDir(effectiveLoraDir()),
		"downloadSource":      dsrc,
...
func (a *App) SaveModelConfig(modelID string, config ModelConfig) error {
	...
	// LoRA adapter refs ride the same config struct, but not every writer
	// populates them: the frontend base-param form and the auto-tuner send
	// configs without the field. A nil slice means "not provided" — preserve
	// the persisted refs; an empty non-nil slice (explicit []) clears them.
	// The LoRA tab itself persists through SetLoraAdapters, which always
	// sends a non-nil slice.
	modelConfigsMu.Lock()
	if config.LoraAdapters == nil {
		if prev, ok := cachedModelConfigs[modelID]; ok && prev.LoraAdapters != nil {
			config.LoraAdapters = prev.LoraAdapters
		}
	}
...
func (a *App) ListLoraAdapters() ([]LoraInfo, error) {
	return scanLoraAdapters(), nil
}
...
func (a *App) GetLoraConfig(modelID string) ([]LoraRef, error) {
	modelConfigsMu.Lock()
	cfg, ok := cachedModelConfigs[modelID]
	modelConfigsMu.Unlock()
	refs := cfg.LoraAdapters
	if !ok || refs == nil {
		return []LoraRef{}, nil
	}
	out := make([]LoraRef, len(refs))
	copy(out, refs)
	return out, nil
}
...
func (a *App) SetLoraAdapters(modelID string, refs []LoraRef) error {
	if modelID == "" {
		return errors.New(tr("模型 id 不能为空", "model id cannot be empty"))
	}
	seen := make(map[string]bool, len(refs))
	cleaned := make([]LoraRef, 0, len(refs))
	for _, ref := range refs {
		...
		if seen[key] {
			return fmt.Errorf(tr("LoRA 适配器 %q 重复", "duplicate LoRA adapter %q"), ref.Name)
		}
		seen[key] = true
		cleaned = append(cleaned, LoraRef{Name: ref.Name, Scale: ref.Scale, Enabled: ref.Enabled})
	}
...
func (a *App) ApplyLoraRuntime(modelID string) error {
	port := getServerPort()
	if port == 0 {
		return errors.New(tr("llama-server 未运行", "llama-server is not running"))
	}
	// Accept either the scanned display name or the router alias, mirroring
	// resolveDirectModel's addressing rules.
	models := scanModels()
	var found *ModelInfo
	for i := range models {
		if models[i].Name == modelID || models[i].Alias == modelID {
			found = &models[i]
			break
		}
	}
	if found == nil {
		return fmt.Errorf(tr("未找到模型: %s", "model not found: %s"), modelID)
	}

	modelConfigsMu.Lock()
	cfg, ok := cachedModelConfigs[found.Name]
	modelConfigsMu.Unlock()
	if !ok {
		cfg = defaultModelConfig()
	}
	refs := enabledLoraRefs(cfg)

	// Desktop router mode addresses the adapter list per model child; direct
	// mode serves exactly one process with no model selector.
	modelParam := ""
	if platformGOOS != "android" {
		modelParam = found.Alias
	}

	loaded, err := fetchLoraAdapters(port, modelParam)
	if err != nil {
		if errors.Is(err, ErrLoraUnsupported) {
			return errors.New(tr("当前 llama-server 不支持 /lora-adapters 接口，设置将在下次启动服务时生效", "this llama-server has no /lora-adapters endpoint; settings take effect the next time the service starts"))
		}
		return fmt.Errorf(tr("读取运行中适配器失败（设置将在下次启动服务时生效）: %w", "failed to read running adapters (settings take effect on next service start): %w"), err)
	}
	if len(refs) == 0 && len(loaded) == 0 {
		return errors.New(tr("服务未加载任何适配器，无需热更", "the service has no adapters loaded; nothing to apply"))
	}

	// Match loaded entries to persisted refs by adapter file name (the path
	// llama-server reports is exactly what it was started with — the bare
	// file name for MyLlama-managed servers). Loaded entries with no matching
	// ref are simply not posted: upstream resets unmentioned scales to 0,
	// which disables them as intended.
	index := make(map[string]int64, len(loaded))
	for _, e := range loaded {
		index[strings.ToLower(filepath.Base(e.Path))] = e.ID
	}
	sets := make([]routerLoraSet, 0, len(refs))
	matched := 0
	for _, ref := range refs {
		if id, ok := index[strings.ToLower(ref.Name)]; ok {
			sets = append(sets, routerLoraSet{ID: id, Scale: ref.Scale})
			matched++
		}
	}
	if matched == 0 {
		return errors.New(tr("运行中的服务未加载这些适配器（需重启服务后生效）", "the running service has none of these adapters loaded (restart the service to apply)"))
	}
	if err := applyLoraAdapters(port, sets); err != nil {
		if errors.Is(err, ErrLoraUnsupported) {
			return errors.New(tr("当前 llama-server 不支持运行时适配器更新，设置将在下次启动服务时生效", "this llama-server does not support runtime adapter updates; settings take effect the next time the service starts"))
		}
		return fmt.Errorf(tr("运行时应用失败（设置将在下次启动服务时生效）: %w", "runtime apply failed (settings take effect on next service start): %w"), err)
	}
	addServerLog(fmt.Sprintf("[INFO] LoRA adapters hot-applied for model %q (%d active)", found.Name, matched))
	return nil
}
