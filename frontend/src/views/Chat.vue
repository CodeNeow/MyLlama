  ...
  >
    ...
    <div class="sticky-top">
      ...
      <div class="chat-toolbar">
        ...
        <!-- Params panel extracted to ChatParamsPanel.vue (behavior-neutral
             refactor): the anchored desktop popover, the phone bottom sheet /
             tablet modal Teleport and their scoped styles moved with it. The
             visibility flag stays here (showParams): the gear button toggles
             it and the document-level click handler closes it; the overlay dim
             reports back through the close emit. The read-only preset summary
             arrives as props — the generation preset state stays on the page
             (toolbar picker, manager dialog and send()'s sampling override all
             read it). -->
        <ChatParamsPanel
          :show="showParams"
          :mobile-tier="isMobileTier"
          :tablet-tier="isTabletTier"
          :preset-label="currentGenPresetLabel"
          :preset-summary="presetSummaryText"
...
import { messages, selectedModel, streaming, chatAbortController, persistChat, reconcileSelectedModel, chatParams, type ChatParams } from '../lib/chatState'
...
/* (The params popover skin (design frame ②) moved to ChatParamsPanel.vue
...
/* (The phone params sheet / tablet modal overlay skin (design frame ⑤) moved
   to ChatParamsPanel.vue with the Teleported markup.) */
...
@media (min-width: 768px) and (max-width: 1099px) {
  /* (Frame A⑤: the params overlay modal-card rules moved to
     ChatParamsPanel.vue with the Teleported markup.) */
