...
**PC service not started? Start it remotely with one tap**: if the PC's MyLlama app is running but its llama-server is not (typically even Retry cannot connect), the warning bar also offers "**Start service remotely**" — tapping it asks the PC's control plane (fixed port **1900**) to bring llama-server up, exactly as if someone pressed "Start service" at the PC. Preconditions (all configured on the PC under Settings → LAN Pairing → Share this machine's service): the "**Allow phone to start the service remotely**" switch on, an **API key** set (remote start must carry it — without a key the PC refuses outright), and the PC app kept running; the switch **takes effect after restarting the PC app**. Outcomes: started, or the service was already running → "Started remotely, connecting…" and the model list recovers on its own shortly; the PC has the switch off or no API key → a hint to enable it on the PC; the PC app itself is not running → remote start is impossible (someone has to attend the PC).
