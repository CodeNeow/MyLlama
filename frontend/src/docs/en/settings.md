...
**Server Access Scope**: `Local (127.0.0.1)` keeps the service to this machine; `LAN (0.0.0.0)` opens it to devices on the same network. Changes apply the next time the service starts. The "LAN" option is offered on desktop only; phones stay local and act solely as the chatting side connecting to other computers.
...
**LAN Pairing**: one card carrying the two directions of one pairing.

- **Share this machine's service** (serving side): while the access scope is "LAN", this section shows a green "Ready" status line and lists this machine's LAN addresses, the service port and the API-key status, each with one-tap copy; on desktop it also renders the **pairing QR code** (a `myllama://pair` link carrying the address, port and API key) — another MyLlama scans it and imports the whole pairing in one go. With several LAN addresses a small picker chooses which one the code encodes; "Refresh QR code" re-renders with the next address, "Copy link" copies the full payload to the clipboard. Without a local API key the status line suggests setting one first (the peer can still connect keyless). **The QR code carries this machine's service address and API key — never show it to untrusted parties.** While the scope is "Local", the section shows an amber warning (the QR stays visible, but phones cannot connect through it) with a one-tap "Switch to LAN" button. On Android the whole section is replaced by a single muted line — the phone runs models locally only and does not serve other devices. Desktop also offers the "**Allow phone to start the service remotely**" switch: when on, the app's control plane (fixed port **1900**) listens on the LAN instead of loopback only, so a phone can pull up this machine's llama-server from its chat page when connecting fails. Security semantics: off by default; on a LAN-bound control plane every gated endpoint **requires a token** (either the `LLAMA_DESKTOP_CONTROL_TOKEN` env var or this machine's API key, compared in constant time), and with both unset everything is refused; destructive endpoints such as "stop service" remain loopback-only, always. The switch **takes effect after restarting the app** (the bind happens at app start).
- **Connect to another PC** (chatting side): on Android, "**Scan to import**" reads the QR code in the peer's "Share this machine's service" section and fills in the address, port and API key automatically (the first use asks for the camera permission); on every platform, "**Import from clipboard**" pastes a `myllama://pair` link copied on the PC, or enter the PC address (the host or IP only), the port and the API key set there manually. Importing only fills the form — review the fields, turn on the **Enable remote chat** switch at the top of the section and save, then switch the chat page's "Chat target" to "Remote PC". LAN traffic is plaintext; the API key keeps freeloaders out but does not prevent eavesdropping. See "LAN remote chat" in the Local Chat section for the full steps.
