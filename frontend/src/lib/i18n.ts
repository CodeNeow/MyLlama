...
export const messages: Record<Locale, Record<string, string>> = {
  zh: {
    ...
    'chat.modelSwitched': '原模型已不可用，已切换到 {name}',
    'chat.pickAfterDownload': '暂无可用模型，请先下载一个模型',
    ...
    // Chat target switcher (LAN remote tier): local service vs the paired PC
    'chat.target': '对话目标',
    ...
    'settings.apiKeyDesc': '配置后调用推理接口需携带该 Bearer Token；留空表示无鉴权（与现状一致）；修改后需重启服务生效',
    ...
    // Saved-while-running restart prompt (issue #32)
    'settings.apiKeySavedTitle': 'API Key 已保存',
    'settings.apiKeySavedBody': '正在运行的服务仍在使用启动时的密钥，本次修改将在服务重启后生效。',
    'settings.apiKeyRestartNow': '立即重启',
    'settings.apiKeyLater': '稍后',
    'settings.apiKeyRestartFailed': '重启服务失败，请稍后重试',
    ...
    'settings.remoteChat.port': '端口',
    'settings.remoteChat.apiKey': 'API Key（可选）',
    'settings.remoteChat.apiKeyPh': '电脑端设置过的 API Key，留空表示无鉴权',
    'settings.remoteChat.save': '保存',
    'settings.remoteChat.saving': '保存中...',
    'settings.remoteChat.saved': '✓ 已保存',
    'settings.remoteChat.errHostEmpty': '请填写电脑地址',
    'settings.remoteChat.errHostScheme': '地址只需填写主机名或 IP，不含 http:// 或路径',
  ...
  en: {
    ...
    'api.restart': 'Restart',
    'api.restartFailed': 'Failed to restart the server: {msg}',
    ...
    'chat.modelSwitched': 'Previous model is gone, switched to {name}',
    'chat.pickAfterDownload': 'No models available — download one first',
    ...
    'settings.apiKey': 'API Key',
    'settings.apiKeyDesc': 'When set, inference API requests must carry this bearer token; leave empty for no authentication (current behavior); changes apply after a service restart',
    ...
    'settings.apiKeySavedBody': 'The running service keeps the key it was started with; this change takes effect after a service restart.',
    'settings.apiKeyRestartNow': 'Restart now',
    'settings.apiKeyLater': 'Later',
    'settings.apiKeyRestartFailed': 'Failed to restart the service, please try again later',
    ...
    'settings.remoteChat.port': 'Port',
    'settings.remoteChat.apiKey': 'API Key (optional)',
    'settings.remoteChat.apiKeyPh': 'The key set on the PC; leave empty for no auth',
    'settings.remoteChat.save': 'Save',
    'settings.remoteChat.saving': 'Saving...',
    'settings.remoteChat.saved': '✓ Saved',
    'settings.remoteChat.errHostEmpty': 'Enter the PC address',
    'settings.remoteChat.errHostScheme': 'Enter the host or IP only — no http:// prefix or path',
